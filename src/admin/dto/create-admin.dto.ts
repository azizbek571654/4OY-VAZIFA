...
export class CreateAdminDto implements Partial<Admin> {
  ...
  @MinLength(3, {
    message: 'Foydalanuvchi nomi kamida 3 ta belgidan iborat bolishi kerak',
  })
  ...
  @ApiProperty({
    example: 'Secure@123',
    description:
      'Kuchli parol: katta va kichik harflar, raqamlar va maxsus belgilarni oz ichiga olgan',
  })
  ...
  @Matches(/(?=.*[a-z])/, {
    message: 'Parolda kamida 1 ta kichik harf bolishi kerak',
  })
  @Matches(/(?=.*[A-Z])/, {
    message: 'Parolda kamida 1 ta katta harf bolishi kerak',
  })
  @Matches(/(?=.*[!@#$%^&*])/, {
    message: 'Parolda kamida 1 ta maxsus belgi (!@#$%^&*) bolishi kerak',
  })
  password: string;

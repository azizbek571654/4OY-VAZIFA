...
export class BotService {
  ...
  async onStop(ctx: Context) {
    try {
      ...
      const user = await this.botModel.findByPk(user_id);
      if (!user) {
        await ctx.replyWithHTML(`siz ozi royhatdan otmagansiz  `, {
          ...Markup.removeKeyboard(),
        });
      }else if (user.status){
        user.status = false
        ...
        await ctx.replyWithHTML("siz vaqtinch botdan chiqib kedingiz", {
          ...Markup.keyboard([["/start"]]).resize(),
        });
      }
    } catch (error) {
...
async sentOTP(phone_number: string, OTP:string): Promise<boolean | undefined>{
    try {
        ...
        await this.bot.telegram.sendMessage(user.user_id, `varify code ${OTP}`)

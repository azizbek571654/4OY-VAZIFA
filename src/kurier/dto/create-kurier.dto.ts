...
export class CreateKurierDto implements Partial<Kurier> {
  @ApiProperty({ example: 'Ali Valiyev', description: 'Kurьerning to‘liq ismi' })
  ...
  @ApiProperty({ example: '+998901234567', description: 'Kurьer telefon raqami (+998 bilan boshlanishi shart)' })
  @IsString({ message: 'Telefon raqam satr bolishi kerak' })
  @Matches(/^\+998\d{9}$/, {
    message: 'Telefon raqam +998 bilan boshlanadigan 9 xonali bolishi kerak',
  })
  phone: string;
  @ApiProperty({ example: 'kurier@example.com', description: 'Kurьerning email manzili' })
  @IsEmail({}, { message: 'Email notogri formatda' })
  email: string;
  @ApiProperty({ example: 'active', description: 'Kurьer holati (masalan: active, pending)' })
